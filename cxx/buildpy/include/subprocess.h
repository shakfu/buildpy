...
enum subprocess_option_e {
    ...
    // The child process should inherit the environment variables of the
    // parent.
    ...
    // Enable the child process to be spawned with no window visible if
    // supported
    // by the platform.
    subprocess_option_no_window = 0x8,

    // Search for program names in the PATH variable. Always enabled on
    // Windows.
    // Note: this will **not** search for paths in any provided custom
    // environment
...
/// this process. The last element must be NULL to signify the end of the
/// array. The memory backing this parameter only needs to persist until this
/// function returns.
...
///
/// If the process was created with the
/// subprocess_option_combined_stdout_stderr option bit set, this function will
/// return NULL, and the subprocess_stdout function should be used for both the
...
#pragma warning(pop)
    ...
    *rd = CreateNamedPipeA(
        name, pipeAccessInbound | fileFlagOverlapped, pipeTypeByte | pipeWait,
        1, 4096, 4096, SUBPROCESS_NULL,
                         ...
                         const char* const process_cwd)
{
    ...
    if (options & subprocess_option_enable_async) {
        out_process->hEventOutput = CreateEventA(
            SUBPROCESS_PTR_CAST(LPSECURITY_ATTRIBUTES, &saAttr), 1, 1,
            SUBPROCESS_NULL);
...
#pragma clang diagnostic pop
    ...
    if (subprocess_option_combined_stdout_stderr
        ...
        // Map the write end to stdout
        if (0
            != posix_spawn_file_actions_adddup2(&actions, stderrfd[1],
                                                STDERR_FILENO)) {
            posix_spawn_file_actions_destroy(&actions);
            return -1;
        }
...
int subprocess_terminate(struct subprocess_s* const process)
{
    ...
    windows_call_result = TerminateProcess(process->hProcess,
    ...
    if (!ReadFile(handle, buffer, size, &bytes_read,
        ...
        if (error == errorIoPending) {
            if (!GetOverlappedResult(
                    handle, SUBPROCESS_PTR_CAST(LPOVERLAPPED, &overlapped),
                    &bytes_read, 1)) {
...
int subprocess_alive(struct subprocess_s* const process)
{
...
#else
    {
        ...
        if (!is_alive) {
            ...
            // Since we've already successfully waited on the process, we need
            // to wipe the child now.
            process->child = 0;
